...
* The initial **Status** set by an import is `Open`. It will be set to `Resolved` if the [same observation](../usage/import_observations.md#import-algorithm) is not found in a subsequent import. On the other hand, resolved observations are set back to the status `Open` if they reappear in a later import.  
    ...
    `Open`, `Affected` and `In review` are the so-called *active* status, because observations in these status need attention. Observations in other status typically don't need attention anymore. 

* The **Priority** *(not shown in the screenshot)* is a number between 1 and 10, to help prioritizing the order of assessing observations. It can be set by [manual assessments](../usage/assess_observations.md) or [rego rules](../usage/rule_engine.md#rego-rules).
